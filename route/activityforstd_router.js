  ...
  try {
    ...
    var statereq =  await new Promise((resolve,rejects)=>{
     var {reqtype,eventtype,durationEventTime,firstdate,lastdate} = req.body
      dbConnectionn.query('INSERT INTO request (idRequest, Username, idType_req, ID_event,file_img,file_pdf,Status_req,hour,start_date,end_date  ) VALUE (?,?,?,?,?,?,?,?,?,?)' ,[getuidf(),req.cookies.sslg.studentID,reqtype,eventtype,img,pdf,"1",durationEventTime,firstdate,lastdate]
